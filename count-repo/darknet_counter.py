...
net = cv2.dnn.readNet(model="yolo/yolov4-tiny.weights", config="yolo/yolov4-tiny.cfg", framework="darknet")
...
def postprocess(frame, outs, trackers):
    ...
    indices = cv2.dnn.NMSBoxes(boxes, confidences, args.get('confidence', 0.4), args.get('confidence', 0.4))
    for i in indices:
        i = i[0]
        box = boxes[i]
        left = box[0]
        ...
        rect = dlib.rectangle(int(left), int(top), int(left+width), int(top+height))
...
while True:
    ...
    if totalFrames % args["skip_frames"] == 0:
        ...
        trackers = postprocess(frame, detections,trackers)
    ...
    # moving 'up' or 'down'
    if args.get('line', 'horizontal') == 'vertical':
        cv2.line(frame, (W // 2, 0), (W // 2, H), (0, 255, 255), 2)
    else: cv2.line(frame, (0, H // 2), (W, H // 2), (0, 255, 255), 2)
    ...
    # frame
    info = [
        ("Total in room", totalUp-totalDown),
        ("Status", status)
    ]
